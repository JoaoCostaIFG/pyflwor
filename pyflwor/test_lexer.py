...
class TestLexer(unittest.TestCase):
    ...
    def test_FLOAT(self):
        ...
        tokens = [
            ...
            token("NUMBER", 0.2, 4, 1),
            token("NUMBER", 2.3e4, 7, 1),
            token("NUMBER", 0.2, 13, 1),
    ...
    def test_chrs(self):
        for typ, value in [
            (attr[2:], getattr(lexer.Lexer, attr))
            for attr in dir(lexer.Lexer)
            if attr[:2] == "t_"
            and isinstance(getattr(lexer.Lexer, attr), str)
            and attr[2:] != "ignore"
        ]:
            if value[0] == "\\":
                value = value[1:]
