        ...
        if attr.callchain:
            for call in attr.callchain:
                ...
                for param in call.params:
                    if (
                        isinstance(param, type(value))
                        and value.__code__ == param.__code__
                        or isinstance(param, type(value))
                        and hasattr(param, "__objquery__")
                    ):
                        p.append(param(objs))
                    else:
                        p.append(param)
    ...
    def value(objs):
        ...
        for attr in attribute_list[1:]:
            if hasattr(obj, attr.name):
                ...
                raise Exception(
                    "object %s did not have attr %s" % (str(obj), attr.name)
                )
        return obj
...
def booleanOperator(op):
    ...
    """
    if op == "and":
        return lambda x, y, objs: x(objs) and y(objs)
    if op == "or":
        return lambda x, y, objs: x(objs) or y(objs)
    raise Exception("operator %s not found" % op)
                    ...
                    if not isinstance(v, str) and hasattr(v, "__iter__"):
                        for z in v:
                            ...
                            if i + 1 == len(attrs):
                                ...
                                add(queue, u, next, i)  # otherwise add to the queue
    ...
    def sequence(objs):
        ...
        if collecting:
            ...
            if not r:
                ...
                if direction == "ASCD":
                    r = sorted(r, key=keyfunc)
                else:
                    r = sorted(r, key=keyfunc, reverse=True)
            return tuple(r)
    ...
    def flwr_function(objs):
        def function(*args):
            if len(args) != len(params):
                raise RuntimeError(
                    "Got wrong number of params expected %d got %d"
                    % (len(params), len(args))
                )
            namespace = dict(objs)
