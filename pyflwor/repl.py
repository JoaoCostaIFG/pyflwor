...
class REPL(object):
    def __init__(self, objects, queries=None):
        if not objects:
            raise Exception(
                "objects was empty, you must supply a query context (ie. objects to query...)"
            )
    ...
    def exe(self, prompt):
        def query(cmds, args):
            ...
            def save_exec(cmds, args):
                ...
                if args.count(" ") < 2:
                    raise Exception(
                        "Must supply both format, file location and a query."
                    )
