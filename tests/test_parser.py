...
class TestParser(unittest.TestCase):
    ...
    def test_where_parens_bool(self):
        pyflwor.compile(
            "hello[((a and b) and not (a or b) or not (a and b)) and not (not a or b)]"
        )
    ...
    def test_where_quantified(self):
        ...
        self.assertRaises(
            SyntaxError, pyflwor.compile, "hello[every x in <asdf> statisfies (x)]"
        )
        self.assertRaises(
            SyntaxError, pyflwor.compile, "hello[every x in <asdf> statisfies x]"
        )
        ...
        pyflwor.compile(
            "hello[some x in {for x in <asdf> return x} satisfies (x == y)]"
        )
        pyflwor.compile(
            "hello[some x in {for x in <asdf> return x} satisfies (x and not y(1,2))]"
        )
    ...
    def test_flwr(self):
        ...
        pyflwor.compile(
            "for x in <asfd>, z in <asdf> let y = <x/asdf> let x = <adf> return x"
        )
        pyflwor.compile(
            """for x in <asfd>, z in <asdf>
            let y = <x/asdf>
            let x = <adf>
        ...
        pyflwor.compile(
            """for x in <asfd>, z in <asdf>, y in <asdf/asdf>
            let y = <x/asdf>
            let x = <adf>
            where every x in <y> satisfies (q == z and (<y> is not <z>))
            return 'asdf':asdf, "one":1, "2.0":2.0"""
    ...
    def test_flwr_attrvalue(self):
        pyflwor.compile(
            """for x in <asfd>, z in <asdf>, y in sdaf.asdf(asdf, asdf)[1]
            let y = <x/asdf>, y1 = <Afd>, y2 = <asdf>, y3 = <asdf>
            let x = <adf>
            where every x in <y> satisfies (q == z and (<y> is not <z>))
            return 'asdf':asdf, "one":1, "2.0":2.0"""
...
class TestParser2(unittest.TestCase):
    ...
    def test_in_list2(self):
        result = pyflwor.execute(
            "res[test_elt in ['foo','bar']]", {"res": True, "test_elt": "foo"}
        )
        self.assertTrue(bool(result))

    def test_in_list3(self):
        result = pyflwor.execute(
            "res[test_elt in ['foo','bar']]", {"res": True, "test_elt": "bazzzz"}
        )
        self.assertFalse(bool(result))

    def test_not_in_list4(self):
        result = pyflwor.execute(
            "res[test_elt not in ['foo','bar']]", {"res": True, "test_elt": "bazzzz"}
        )
        self.assertTrue(bool(result))

    def test_not_in_list5(self):
        result = pyflwor.execute(
            "res[test_elt not in ['foo','bar']]", {"res": True, "test_elt": "foo"}
        )
        self.assertFalse(bool(result))
